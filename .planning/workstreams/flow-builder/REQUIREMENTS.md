...
- [ ] **SCHEMA-01**: Admin can create flows with name, description, status (draft/active/paused/archived), priority, display type, trigger, frequency, and conditions
- [ ] **SCHEMA-02**: Admin can define flow steps with position ordering, title, and elements (jsonb array of typed element objects)
- [ ] **SCHEMA-03**: Admin can define branches from single-choice elements that route to specific steps based on answer value
- [ ] **SCHEMA-04**: User flow responses are recorded with start/complete timestamps, last step for resumability, and per-element answer storage
- [ ] **SCHEMA-05**: Flow analytics events are recorded (shown, started, step_completed, completed, skipped, dismissed) with user and step references
- [ ] **SCHEMA-06**: Profiles table has birthday date column for condition evaluation
- [ ] **SCHEMA-07**: RLS policies enforce admin/moderator write access and user-own-data read/write on flow_responses
