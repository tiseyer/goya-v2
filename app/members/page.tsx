...
export default function MembersPage() {
  const [allMembers, setAllMembers] = useState<Member[]>(staticMembers);

  // Fetch school members and merge with static members
  useEffect(() => {
    fetchSchoolMembers().then(schoolMembers => {
      if (schoolMembers.length > 0) {
        const existingIds = new Set(staticMembers.map(m => m.id));
        const newSchools = schoolMembers.filter(s => !existingIds.has(s.id));
        setAllMembers([...staticMembers, ...newSchools]);
      }
    }).catch(() => { /* school fetch failed — use static data only */ });
  }, []);
