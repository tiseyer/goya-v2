...
export default function ProfileHero({
  ...
  return (
    <div>
      {/* ── Hero Banner ─────────────────────────────────────────────────────── */}
      <div
        className={`relative overflow-hidden h-[200px] sm:h-[240px] ${!profile.cover_image_url ? 'bg-[var(--goya-primary)]' : ''}`}
      >
        ...
        <div className="-mt-16 mb-4">
          <div className="w-[120px] h-[120px] rounded-full ring-4 ring-white overflow-hidden bg-slate-200 shadow-lg flex items-center justify-center">
            {profile.avatar_url ? (
              ...
              <span className="text-3xl font-bold text-white bg-[var(--goya-primary)] w-full h-full flex items-center justify-center">
        ...
        {(hasLanguages || formatPill) && (
          <div className="flex flex-wrap gap-2 mb-4">
            {hasLanguages && profile.languages!.map((lang) => (
              <span
                key={lang}
                className="rounded-full bg-[var(--goya-primary)]/10 text-[var(--goya-primary)] px-3 py-1 text-sm"
              >
                {lang}
              </span>
            ))}
