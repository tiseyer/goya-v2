...
export default function FaqRow({ item, isExpanded, onExpand, onDelete, onUpdate }: FaqRowProps) {
  ...
  async function handleSave() {
    setIsSaving(true)
    const result = await updateFaqItem(item.id, question, answer, category.trim() || undefined)
    setIsSaving(false)
    if (result.success) {
      onUpdate(item.id, { ...item, question, answer, category: category.trim() || null })
      onExpand(null)
    }
  }
            ...
            <div className="bg-slate-50 border-t border-slate-200 px-4 py-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Membership, Payment..."
                  className="w-full border border-[#E5E7EB] rounded-lg px-3 py-2 text-sm text-[#374151] focus:outline-none focus:ring-2 focus:ring-[#00B5A3] bg-white"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold uppercase tracking-widest text-[#6B7280] mb-1">
