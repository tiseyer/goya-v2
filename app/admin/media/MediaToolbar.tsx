...
export default function MediaToolbar({
  ...
  return (
    // POLISH-04: Two-row layout on mobile (search full-width top row, filters scrollable bottom row)
    // Single row on sm+ breakpoint as before.
    <div className="border-b border-slate-200 bg-white shrink-0 w-full">
      ...
      <div className="flex flex-col sm:hidden">
        ...
        {/* Row 2: filters + sort + view toggle (horizontally scrollable) */}
        <div className="flex items-center gap-2 px-3 pb-2.5 overflow-x-auto">
          <div className="relative shrink-0">
            <select value={type} onChange={(e) => onTypeChange(e.target.value as 'all' | 'images' | 'pdfs' | 'videos')} aria-label="Filter by file type" className={SELECT_CLS}>
          ...
          <div className="relative shrink-0">
            <select value={date} onChange={(e) => onDateChange(e.target.value as 'all' | 'today' | 'week' | 'month')} aria-label="Filter by date" className={SELECT_CLS}>
          ...
          <div className="relative shrink-0">
            <select value={by} onChange={(e) => onByChange(e.target.value as 'all' | 'team' | 'members')} aria-label="Filter by uploader" className={SELECT_CLS}>
          ...
          <div className="relative shrink-0">
            <select value={sort} onChange={(e) => onSortChange(e.target.value as 'newest' | 'oldest' | 'name' | 'size')} aria-label="Sort order" className={SELECT_CLS}>
          ...
          <div className="flex items-center gap-1 border border-slate-200 rounded-lg p-0.5 shrink-0">
            <button onClick={() => onViewModeChange('grid')} aria-label="Grid view" aria-pressed={viewMode === 'grid'} className={['w-7 h-7 flex items-center justify-center rounded transition-colors duration-150 cursor-pointer', viewMode === 'grid' ? 'bg-primary/10 text-primary' : 'text-slate-400 hover:text-slate-600'].join(' ')}><GridIcon /></button>
            <button onClick={() => onViewModeChange('list')} aria-label="List view" aria-pressed={viewMode === 'list'} className={['w-7 h-7 flex items-center justify-center rounded transition-colors duration-150 cursor-pointer', viewMode === 'list' ? 'bg-primary/10 text-primary' : 'text-slate-400 hover:text-slate-600'].join(' ')}><ListIcon /></button>
          </div>
        </div>
      </div>

      {/* ── sm+: single-row layout (original) ── */}
