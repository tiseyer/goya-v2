...
export default function FolderSidebar({
  ...
  // ── Section expansion state (independent of active bucket) ─────────────────
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>(() => {
    // Start with the active bucket expanded
    const initial: Record<string, boolean> = {};
    if (activeBucket) initial[activeBucket] = true;
    return initial;
  });
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
          {SIDEBAR_SECTIONS.map(section => {
            ...
            // Expansion is independent — tracked in expandedSections state
            ...
            return (
              <div key={section.key} className="space-y-0.5">
                ...
                <div className="flex items-center group/section">
                  <button
                    onClick={() => {
                      onBucketSelect(section.key);
                      // Toggle expansion independently; auto-expand on first select
                      if (isSectionActive && section.hasFolders) {
                        toggleSection(section.key);
                      } else if (!isExpanded && section.hasFolders) {
                  ...
                  {section.key === 'media' && isSectionActive && isExpanded && (
                    <button
                      onClick={() => {
