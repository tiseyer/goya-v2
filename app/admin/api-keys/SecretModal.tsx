...
export default function SecretModal({ mode, secret, onClose, onSaved }: SecretModalProps) {
  ...
  useEffect(() => {
    if (mode === 'edit' && secret) {
      ...
      // Load the secret to get metadata — value stays empty/masked per SECR-03
      getSecret(secret.id).then((result) => {
        if (result.success) {
          // We have the data — name/category/description already set from SecretListItem
          // Value intentionally left empty; placeholder guides the user
        }
      })
    }
  }, [mode, secret])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[#374151] mb-1.5">
              Value {mode === 'create' && <span className="text-red-500">*</span>}
            </label>
            <input
              type="password"
              value={value}
