  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <h1 className="text-2xl font-bold text-[#1B3A5C]">
            {tab === 'categories' ? 'Categories' : 'Courses'}
          </h1>
          {tab !== 'categories' && (
      ...
      <div className="flex gap-6 border-b border-[#E5E7EB] mb-6">
        <Link
          href="/admin/courses"
          className={`pb-3 text-sm transition-colors ${tab !== 'categories' ? 'border-b-2 border-[#4E87A0] text-[#1B3A5C] font-semibold' : 'text-[#6B7280] hover:text-[#374151]'}`}
        >
          Courses
        </Link>
        <Link
          href="/admin/courses?tab=categories"
          className={`pb-3 text-sm transition-colors ${tab === 'categories' ? 'border-b-2 border-[#4E87A0] text-[#1B3A5C] font-semibold' : 'text-[#6B7280] hover:text-[#374151]'}`}
        >
          Categories
        </Link>
                        ...
                        <tr key={course.id} className={`transition-colors ${isDeleted ? 'bg-red-50/30 hover:bg-red-50/60' : 'hover:bg-slate-50'}`}>
                          ...
                          <td className="px-4 py-3 max-w-xs">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${CATEGORY_BADGE[course.category as string] ?? 'bg-slate-100 text-slate-600'}`}>
