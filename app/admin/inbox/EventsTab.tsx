...
export default function EventsTab({ events: initialEvents }: EventsTabProps) {
  ...
  const filtered = events.filter(e => e.status === activeSubTab)
  ...
  async function handleReject(eventId: string) {
    ...
    setBusy(eventId)
    const result = await rejectEvent(eventId, rejectReason)
    if (result.success) {
      setEvents(prev =>
        prev.map(e =>
          e.id === eventId
      ...
      {sorted.length === 0 ? (
        <div className="p-16 text-center">
          <p className="font-semibold text-slate-700 mb-1">
            No {activeSubTab === 'pending_review' ? 'pending' : activeSubTab} events
          </p>
          <p className="text-sm text-slate-400">
            {activeSubTab === 'pending_review'
              ? 'New event submissions will appear here for review.'
              : `${STATUS_LABELS[activeSubTab]} events will appear here.`}
          </p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          ...
          <div className="hidden md:grid grid-cols-[2fr_1.2fr_0.8fr_0.8fr_0.7fr_auto] gap-4 px-6 py-3 border-b border-slate-100 bg-slate-50 min-w-[800px]">
            {['EVENT TITLE', 'SUBMITTED BY', 'CATEGORY', 'DATE', 'SUBMITTED', 'ACTIONS'].map(col => (
                ...
                <div key={event.id}>
                  ...
                  <div className="px-6 py-4 grid grid-cols-1 md:grid-cols-[2fr_1.2fr_0.8fr_0.8fr_0.7fr_auto] gap-4 items-center">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className={`inline-block text-xs px-2.5 py-1 rounded-full font-semibold capitalize mt-1 ${STATUS_STYLES[event.status] ?? 'bg-slate-50 text-slate-700 border border-slate-200'}`}>
                    ...
                    <div className="min-w-0">
                      <p className="text-sm text-slate-600 truncate">
                        {event.submitter_name ?? 'Unknown user'}
                      </p>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {event.status === 'pending_review' && (
                        <>
                          ...
                          <button
                            onClick={() => { setRejectOpen(event.id); setRejectReason('') }}
                            disabled={isBusy}
                      ...
                      {event.status === 'rejected' && event.rejection_reason && (
                        <p className="text-xs text-slate-400 max-w-[200px] truncate" title={event.rejection_reason}>
