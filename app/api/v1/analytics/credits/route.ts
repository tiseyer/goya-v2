...
import { getCreditStats } from '@/lib/api/services/analytics-engagement';
...
const handlers = createApiHandler({
  GET: async (ctx) => {
    ...
    const { data, error } = await getCreditStats({ date_from, date_to });

    if (error) {
      return errorResponse('QUERY_ERROR', 'Failed to fetch credit stats', 500);
    }
