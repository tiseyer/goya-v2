...
import { getMembershipStats } from '@/lib/api/services/analytics';
...
const handlers = createApiHandler({
  GET: async (ctx) => {
    ...
    // Query
    const { data, error } = await getMembershipStats({ date_from, date_to });

    if (error) {
      return errorResponse('QUERY_ERROR', 'Failed to fetch membership stats', 500);
    }
