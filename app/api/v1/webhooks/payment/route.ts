...
import type { WebhookPaymentPayload } from '@/lib/api/services/webhooks';
...
const handlers = createApiHandler({
  POST: async (ctx) => {
    ...
    // Parse body
    let body: WebhookPaymentPayload;
    try {
    ...
    // Process
    const { data, error } = await processWebhookPayment(body);
    if (error || !data) {
    ...
    await ctx.logAudit({
      ...
      target_type: 'webhook',
      description: 'Received payment webhook',
      metadata: { transaction_id: body.transaction_id, status: body.status },
    });
