      ...
      <div className="w-full max-w-[520px]">
              ...
              <div key={s.n} className="flex items-center">
                ...
                <div className="flex flex-col items-center gap-1.5">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                      ...
                      ? 'bg-[#6E88B0] text-white'
                      : active
                      ? 'bg-[#6E88B0] text-white ring-4 ring-[#6E88B0]/20'
                  ...
                  <span className={`text-[10px] font-semibold uppercase tracking-wider transition-colors ${
                    active ? 'text-[#6E88B0]' : done ? 'text-slate-400' : 'text-slate-400'
                  }`}>
                    {s.label}
                  </span>
                </div>
                ...
                {i < STEPS.length - 1 && (
                  <div className="w-16 sm:w-24 mx-2 mb-5">
                    <div className="h-px relative overflow-hidden rounded-full bg-slate-200">
                      <div
                        className="absolute inset-y-0 left-0 bg-[#6E88B0] transition-all duration-500"
              ...
              {step === 1 && (
                <>
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-start gap-4 p-5 rounded-xl border-2 text-left transition-all duration-200 ${
                            selected
                              ? 'border-[#6E88B0] bg-[#eef4f9]'
                              : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50'
                          }`}
                        >
                          {/* Icon */}
                          <div className={`mt-0.5 shrink-0 transition-colors ${selected ? 'text-[#6E88B0]' : 'text-slate-400'}`}>
                            {r.icon}
                          </div>
                          {/* Text */}
                          ...
                          <div className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all mt-0.5 ${
                            selected ? 'bg-[#6E88B0] border-[#6E88B0]' : 'border-slate-300'
                          }`}>
                            {selected && (
                  ...
                  <button
                    disabled={!role}
                    onClick={() => setStep(2)}
                    className="w-full bg-[#6E88B0] text-white py-3.5 rounded-xl font-bold text-sm hover:bg-[#1e3a52] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    Continue &rarr;
                  </button>
                </>
                ...
                <>
                  <div className="mb-7">
                    ...
                    <p className="text-slate-500 text-sm">
                      Joining as{' '}
                      <span className="text-[#6E88B0] font-semibold">{role}</span>
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div className={`mt-0.5 w-5 h-5 rounded flex items-center justify-center border-2 shrink-0 transition-all ${
                        form.agreed ? 'bg-[#6E88B0] border-[#6E88B0]' : 'border-slate-300 group-hover:border-slate-400'
                      }`}
                      ...
                      <span className="text-xs text-slate-500 leading-relaxed pt-0.5">
                        I agree to the{' '}
                        <Link href="/terms" className="text-[#6E88B0] hover:underline">Terms of Use</Link>
                        {' '}and{' '}
                        <Link href="/privacy" className="text-[#6E88B0] hover:underline">Privacy Policy</Link>
                      </span>
                    </label>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 bg-[#6E88B0] text-white py-3.5 rounded-xl font-bold text-sm hover:bg-[#1e3a52] transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
              ...
              {step === 3 && (
                <div className="text-center py-4">
                  ...
                  <div className="relative w-20 h-20 mx-auto mb-7">
                    <div className="absolute inset-0 bg-[#6E88B0]/20 rounded-full animate-ping" style={{ animationDuration: '1.5s', animationIterationCount: 1 }} />
                    <div className="relative w-20 h-20 bg-[#6E88B0] rounded-full flex items-center justify-center shadow-lg shadow-[#6E88B0]/30">
                  ...
                  <div className="flex flex-col gap-3">
                    <Link
                      href="/members"
                      className="bg-[#6E88B0] text-white px-6 py-3.5 rounded-xl font-bold text-sm hover:bg-[#1e3a52] transition-colors inline-flex items-center justify-center gap-2"
                    >
                      Explore Members
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {step < 3 && (
          <p className="text-center text-slate-400 text-xs mt-6">
            ...
            <Link href="/sign-in" className="text-[#6E88B0] hover:underline font-semibold">Sign in</Link>
          </p>
        )}
  ...
  return (
    <Suspense fallback={
      <div className="h-screen flex items-center justify-center bg-[#f8f9fa]">
        <div className="w-8 h-8 border-2 border-[#6E88B0] border-t-transparent rounded-full animate-spin" />
      </div>
    }>
      <RegisterPageInner />
    </Suspense>
