...
export async function getImpersonationState(): Promise<ImpersonationState> {
  ...
  const [{ data: adminProfile }, { data: targetProfile }] = await Promise.all([
    supabaseService.from('profiles').select('id, full_name, role').eq('id', user.id).single(),
    supabaseService.from('profiles').select('id, full_name, role, member_type, mrn').eq('id', targetUserId).single(),
  ])

  // Also get target user's email from auth.users via service client
  const { data: targetAuthUser } = await supabaseService.auth.admin.getUserById(targetUserId)
